...
void onRecvTarget(const std_msgs::Float64MultiArrayConstPtr &Ptr)
{
    jointTarget=Ptr->data;
    startPlan=true;
}
...
int main(int argc,char** argv)
{
    ...
    // init
    ros::init(argc,argv,"test_planner_joint");
    ros::NodeHandle nh;
    ros::Rate rate(10);
    ...
    ros::Subscriber targetSub=nh.subscribe("/planning_target_joint",10,&onRecvTarget);
    ros::Publisher planPub=nh.advertise<moveit_msgs::RobotTrajectory>("/generated_plan_joint",10);
    ...
    while(ros::ok())
    {
        if(startPlan)
        {
            ...
            //IK
            std::vector<double> targetJointValue=jointTarget;
